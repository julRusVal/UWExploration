...
int main(int argc, char** argv){
    ...
    int num_auvs_;
    void setup_rbpf(std::string base_link_custom);
    ...
    if (auv_id == 0)
    {
        ...
        setup_rbpf(self_base_link);
        setup_rbpf(neighbour_right_base_link);
    }
    else if (auv_id == num_auvs_-1)
    {
        ...
        setup_rbpf(self_base_link);
        setup_rbpf(neighbour_left_base_link);
    }
    else
    {
        string self_base_link = "hugin_" + std::to_string(auv_id) + "/base_link";
...
void setup_rbpf(string base_link_custom_){
    // ros::NodeHandle nh("~");
    // ros::NodeHandle nh_mb("~");
    // ros::CallbackQueue rbpf_queue;
    // ros::CallbackQueue mb_queue;
    ...
    // boost::shared_ptr<RbpfSlam> rbpf_multi(new RbpfMultiagent(nh, nh_mb));
    boost::shared_ptr<RbpfSlam> rbpf_multi(new RbpfSlamMultiExtension(nh, nh_mb, base_link_custom_));
}
